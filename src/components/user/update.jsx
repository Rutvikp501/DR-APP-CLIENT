...
const Update = () => {
  ...
  let localStorageLoginID = localStorageData.LoginID 
  ...
  const params = { LoginID: localStorageLoginID}
  const getData = async() =>{
    try {
      
        let result = await axios.post('http://127.0.0.1:5001/user/',params)
        setdata(result.data)
    } catch (error) {
        console.log(error);
    }
  ...
  const handleUpdate = async()=>{
    ...
    try {
        let ans = await axios.patch(`http://127.0.0.1:5001/user/update/${updateID}`,dataObj,config)
        toast({
            title: `Note Updated`,
            status: 'success',
            duration: 2000,
          ...
          data[updatedDataIndex].LoginID = LoginID
    } catch (error) {
        toast({
            title: `Login first1`,
  ...
  const handleUpdateModal = (item)=>{
   // console.log(item);
      onupdateOpen()
    const {Name,Email,LoginID,_id:id} = item
    setName(Name)
    setEmail(Email)
    setLoginID(LoginID)
    setupdateID(id)
  ...
  const handleDelete = async(id)=>{
    ...
    try {
        let ans = await axios.delete(`http://127.0.0.1:5001/user/delete/${id}`,config)
        toast({
            title: `User Deleted`,
            status: 'error',
            duration: 2000,
...
//   }
//   try {
//     let ans = await axios.post('http://127.0.0.1:5001/note/create',dataObj,config)
//     toast({
//       title: `Note created`,
//       status: 'success',
//       duration: 2000,
...
useEffect(()=>{
  ...
  return (
    ...
    <VStack w={{base:'80%',md:'50%'}} m='auto' mt='7' spacing={'5'}>
      ...
      <Text fontSize='md'>Login Id</Text>
      <Input placeholder='Enter category' size='md' value={LoginID} onChange={(e)=>setLoginID(e.target.value)} />
          </ModalBody>
      ...
      </Modal>
      {
        ...
        <Text fontSize='lg' >LoginID:  {item.LoginID}</Text>
